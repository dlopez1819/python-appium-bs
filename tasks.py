...
def run_automation_test(c, m, app, env, appiumServer, device, operatingsystem):
    ...
    if appiumServer == "browserstack": #For browserstack enable parallel running to 5
        ...
        runningCommand = f"python3 -m pytest -m {m} ./src/testcases/{app}/* --workers 5 --tests-per-worker 1 --env ={env} --app={app} --appiumServer={appiumServer} --device='{device}' --os='{operatingsystem}' --reruns 0 --html='./report/html/{m}_{device}_{operatingsystem}.html'&"
